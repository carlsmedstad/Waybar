...
static std::string get_from_desktop_app_info(const std::string &app_id) {
  ...
  if (app_info) return app_info->get_icon()->to_string();
...
static bool image_load_icon(Gtk::Image &image, Glib::RefPtr<Gtk::IconTheme> icon_theme,
  ...
  while (stream >> app_id) {
    ...
    std::transform(lower_app_id.begin(), lower_app_id.end(), lower_app_id.begin(), [](char c) {
      return std::tolower(c);
    });

    std::string icon_name = get_from_icon_theme(icon_theme, app_id);

    if (icon_name.empty()) icon_name = get_from_icon_theme(icon_theme, lower_app_id);
    if (icon_name.empty()) icon_name = get_from_desktop_app_info(app_id);
    if (icon_name.empty()) icon_name = get_from_desktop_app_info(lower_app_id);

    if (icon_name.empty()) continue;
...
void Task::handle_title(const char *title) { title_ = title; }
...
void Task::update() {
  if (!format_before_.empty()) {
    text_before_.set_label(fmt::format(format_before_,
  ...
  if (!format_after_.empty()) {
    text_after_.set_label(fmt::format(format_after_,
                                      fmt::arg("title", title_),
                                      fmt::arg("app_id", app_id_),
                                      fmt::arg("state", state_string()),
                                      fmt::arg("short_state", state_string(true))));
  ...
  if (!format_tooltip_.empty()) {
    button_.set_tooltip_markup(fmt::format(format_tooltip_,
                                           fmt::arg("title", title_),
                                           fmt::arg("app_id", app_id_),
                                           fmt::arg("state", state_string()),
                                           fmt::arg("short_state", state_string(true))));
...
void Taskbar::remove_task(uint32_t id) {
  auto it = std::find_if(
      std::begin(tasks_), std::end(tasks_), [id](const TaskPtr &p) { return p->id() == id; });
...
std::vector<Glib::RefPtr<Gtk::IconTheme>> Taskbar::icon_themes() const { return icon_themes_; }
